...
<Steps>
  <Step title="Project setup">
    Import modules and set up the tick loop.
  </Step>
...
tick event, and grab the local player.
...
A text shadow makes names readable against any background:

```ts
const centerX = (box.mins.x + box.maxs.x) / 2;
