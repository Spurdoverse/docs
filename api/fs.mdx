...
The `fs` global gives scripts a sandboxed file system rooted at `scripts/data`.
No imports required — use `fs.readFile(...)`, `fs.mkdir(...)`, etc. directly.
...
<CardGroup cols={2}>
  <Card title="Reading & writing" icon="file-pen" href="#reading--writing">
    Read, write, and append file contents as UTF-8 strings.
  </Card>
...
Returns the full UTF-8 contents of a file.

```ts
const json = await fs.readFile("profiles/player1.json");
...
```ts
fs.writeFile(path: string, data: string): Promise<true>
fs.writeFileSync(path: string, data: string): true
```
...
```ts
fs.appendFile(path: string, data: string): Promise<true>
fs.appendFileSync(path: string, data: string): true
```
