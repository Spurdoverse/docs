...
The `Entities` global exposes the game's entity list. Every entity is backed
...
<CardGroup cols={2}>
  ...
  <Card title="Entity object" icon="cube" href="#entity-object">
    Schema fields, reserved properties, and the raw-buffer API.
  </Card>
...
for (const player of Entities.getPlayers({ skipLocal: true })) {
    const pos = player.m_vecAbsOrigin;
...
const myPos = Entities.getLocalPlayer()?.m_vecAbsOrigin;
enemies.sort((a, b) =>
    a.m_vecAbsOrigin.distTo(myPos) - b.m_vecAbsOrigin.distTo(myPos)
);
...
// Vectors and angles
const pos      = player.m_vecAbsOrigin;       // Vector
...
### `.controller` (player pawns only)

Available on `C_CSPlayerPawn` and `C_CSObserverPawn`. Returns the
`CCSPlayerController` entity linked to this pawn.
...
const myPos = local.m_vecAbsOrigin;
...
Entities.find("C_CSPlayerPawn")
    ...
    .forEach(enemy => {
        const dist = enemy.m_vecAbsOrigin.distTo(myPos);
        if (dist < bestDist) {
            bestDist = dist;
            closest = enemy;
        }
