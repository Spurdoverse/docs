...
Methods available on entities depending on their class. The engine resolves
methods through the full inheritance chain — a `C_CSPlayerPawn` has access
to all `C_BaseEntity`, `C_BaseModelEntity`, and its own methods.

#### C_BaseEntity (all entities)

| Method | Returns | |
|:-------|:--------|:--|
| `.isEnemy(other)` | `boolean` | Whether `other` is an enemy (respects `mp_teammates_are_enemies`) |
| `.getOrigin()` | [`Vector`](/api/types/vector) | World-space position |
| `.getVelocity()` | [`Vector`](/api/types/vector) | Current velocity |
| `.getBoundingBox()` | `[Vector2, Vector2] \| null` | Screen-space `[mins, maxs]`, or `null` if behind camera |
...
#### C_BaseModelEntity (pawns, weapons, props)

| Method | Returns | |
|:-------|:--------|:--|
| `.getEyePosition()` | [`Vector`](/api/types/vector) | Eye position (origin + view offset) |
| `.isVisible(other)` | `boolean` | Traces from eye position to `other`'s origin |
...
#### C_CSPlayerPawn / C_CSObserverPawn

| Property | Type | |
|:---------|:-----|:--|
| `.controller` | `Entity` | The `CCSPlayerController` linked to this pawn |
...
| Method | Returns | |
|:-------|:--------|:--|
| `.getWeaponData()` | `CCSWeaponBaseVData` | Weapon VData — `damage`, `range`, `spread`, `weaponType`, `price`, and all schema fields |

```ts
const local = Entities.getLocalPlayer();
const weapon = local?.m_hActiveWeapon;
