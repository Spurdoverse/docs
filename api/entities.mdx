...
<Tip>
  The string is read once per model and cached - further calls just
  return a copy from local memory, so it's cheap to call every frame.
</Tip>
...
Returns every bone on the entity as an array. Returns `null` if the
scene, model, or bone array is invalid.
...
bones?.forEach(b => Render.circle3D(b.position, 2, Color.white()));
```
...
Returns a single bone by index. Faster than
[`getBones()`](#getbones) - reads only the 32 bytes for that bone
rather than the full bone array. Returns `null` if the index is out of
range or the model is invalid.

```ts
const head = entity.getBone(8);
if (head) Render.circle3D(head.position, 4, Color.red());
```
...
Returns every hitbox on the entity. Returns `null` on error, or an empty
array `[]` if the model has no hitboxes.
...
for (const hb of entity.getHitboxes() ?? []) {
    Render.line3D(hb.start, hb.end, Color.yellow());
}
```
...
Returns a single hitbox by index (`0 .. hitboxes.length - 1`). Faster
than [`getHitboxes()`](#gethitboxes) - reads only the one bone needed
to resolve `start` / `end`, not the full bone array. Returns `null` if
`id` is out of range, or if the bone or model is invalid.
