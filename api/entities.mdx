...
```ts
entity.getEyePosition(): Vector
```

Returns the eye position (origin + view offset). Available on pawns,
weapons, and props (`C_BaseModelEntity` and subclasses).
...
```ts
entity.isVisible(other: Entity): boolean
```

Traces from this entity's eye position to `other`'s origin. Available
on pawns, weapons, and props (`C_BaseModelEntity` and subclasses).
...
The `CCSPlayerController` linked to this pawn. Available on
`C_CSPlayerPawn` and `C_CSObserverPawn`.
...
`price`, and all schema fields. Available on `C_BasePlayerWeapon`.
