...
<CodeGroup>
```ts Basic aim angle
const aim = Math.calcAngle(myEyePos, enemyHeadPos);
```
...
```ts
const { forward } = Math.angleVectors(myViewAngle);

// Project 1000 units ahead of the player's view
const traceEnd = {
    x: eyePos.x + forward.x * 1000,
    y: eyePos.y + forward.y * 1000,
    z: eyePos.z + forward.z * 1000,
};
```
...
if (success) {
    ...
    render.text({ x: screen.x + 8, y: screen.y - 6 }, "HEAD", Color.red());
}
```
...
```ts
const { success, screen } = Math.screenTransform(worldPos);
...
```ts
const aim = Math.calcAngle(myPos, targetPos);
...
<Tabs>
  <Tab title="Health → color">
    ...
    const color = Color.fromRGBA(r, g, 0, 255);
    ```
  </Tab>
...
const fontSize = Math.lerp(14, 8, eased);

const color = Color.fromRGBA(255, 255, 255, alpha);
render.text(screenPos, name, color, fontSize);
```
...
The engine re-binds `sin`, `cos`, `abs`, `sqrt`, and `atan2` through its
C++ implementations. Identical behavior and precision to the JS originals —
you can use them interchangeably.

| Function | Note |
|:---------|:-----|
| `Math.sin(rad)` | Sine, returns `[-1, 1]` |
| `Math.cos(rad)` | Cosine, returns `[-1, 1]` |
| `Math.abs(val)` | Absolute value |
| `Math.sqrt(val)` | Square root, `NaN` for negatives |
| `Math.atan2(y, x)` | Angle to point `[-π, π]`. **y first, then x** |
